    ...
    fun calculateOffset(availableY: Float): Offset {
        ...
        if(availableY > 0) {
            ...
            (if(state.elements.any { it.behavior == CollapsingBehavior.ALWAYS && it.isCollapsed }) {
                ...
                it.behavior == CollapsingBehavior.ALWAYS || (state.overallScroll >= -10f && it.behavior == CollapsingBehavior.ON_TOP)
            }).also { list ->
    ...
    val nestedScrollConnection = remember(content) {
        object: NestedScrollConnection {
            ...
            override fun onPostScroll(
                ...
                state.overallScroll = kotlin.math.min(state.overallScroll + consumed.y, 0f)
        ...
        content.forEachIndexed { index, pair ->
            ...
            Box(
                modifier = Modifier
                    ...
                    .onSizeChanged { size ->
                        state.elements.getOrNull(index)?.height?.doubleValue = size.height.toDouble()
                    }
                    .offset {
                        IntOffset(
                            x = 0,
                            y = (heightAbove.value + offSetAbove.value + (offSet?.doubleValue ?: 0.0)).roundToInt()
                        )
                    }
